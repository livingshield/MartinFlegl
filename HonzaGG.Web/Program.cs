    ...
    if (!string.IsNullOrWhiteSpace(connectionString))
    {
        try
        {
            ...
            string createTableSql = @"
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='honzagg_leads' AND xtype='U')
                BEGIN
                    CREATE TABLE honzagg_leads (
            ...
            Console.WriteLine("[DB] Tabulka honzagg_leads je připravena.");
        }
        catch (Exception ex)
        {
    ...
    try
    {
        if (!string.IsNullOrWhiteSpace(connectionString))
        {
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                string insertSql = "INSERT INTO honzagg_leads (FullName, Email, Phone, Topic) VALUES (@FullName, @Email, @Phone, @Topic)";
        ...
        if (!string.IsNullOrWhiteSpace(smtpHost) && !string.IsNullOrWhiteSpace(smtpUser) && !string.IsNullOrWhiteSpace(smtpPass) && !string.IsNullOrWhiteSpace(targetEmail))
        {
            ...
            message.From.Add(new MailboxAddress("HonzaGG Web", smtpUser));
            ...
            message.Body = new TextPart("plain")
            {
                Text = $"Dobrý den,\n\nmáte nový lead z webového formuláře:\n\n" +
                       ...
                       $"---\nVygenerováno automaticky systémem pro HonzaGG."
            };
