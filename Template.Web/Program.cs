    ...
    if (!string.IsNullOrEmpty(connectionString))
    {
        try
        {
            ...
            string createTableSql = @"
                IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='template_leads' AND xtype='U')
                BEGIN
                    CREATE TABLE template_leads (
    ...
    try
    {
        ...
        if (!string.IsNullOrEmpty(connectionString))
        {
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                string insertSql = "INSERT INTO template_leads (FullName, Email, Phone, Topic) VALUES (@FullName, @Email, @Phone, @Topic)";
        ...
        if (smtpUser != null && smtpPass != null && targetEmail != null)
        {
            ...
            message.From.Add(new MailboxAddress("Project Template Web", smtpUser));
            message.To.Add(new MailboxAddress("Admin", targetEmail));
            message.Subject = "New Lead: " + lead.FullName;
